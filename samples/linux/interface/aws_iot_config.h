...
#define AWS_IOT_MQTT_CLIENT_ID         "odroid1" ///< MQTT client ID should be unique for every device
#define AWS_IOT_THING_CLIENT_ID        "odroid1thing" ///< Shadow client ID should be unique for every device
#define AWS_IOT_MY_THING_NAME 		   "odroid1" ///< Thing Name of the Shadow this device is associated with
...
#define AWS_IOT_MQTT_RX_BUF_LEN 131072 ///< Any message that comes into the device should be less than this buffer size. If a received message is bigger than this buffer size the message will be dropped.
#define AWS_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS 100 ///< Maximum number of topic filters the MQTT client can handle at any given time. This should be increased appropriately when using Thing Shadow
